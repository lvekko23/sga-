...
# Nuevos colores (Verde/Blanco) y nuevo nombre (SGA)
files = {
...
export const COMPANY_INFO = {
  ...
  address: "Av. San Martín 1234, Luján, Buenos Aires",
...
export const SERVICES = [
  { id: "cucarachas", title: "Cucarachas", description: "Gel sin olor." },
  { id: "roedores", title: "Roedores", description: "Cebos seguros." },
];
""",
    "src/app/page.tsx": """
import { ShieldCheck, Bug, Rat, Droplets } from 'lucide-react';
import { COMPANY_INFO } from '@/lib/constants';

export default function Home() {
  return (
    <div className="flex flex-col gap-16 pb-10 bg-white">
      {/* Hero Section - AHORA VERDE */}
      <section className="relative bg-green-700 text-white py-24 px-4 overflow-hidden">
        ...
        <div className="relative container mx-auto text-center max-w-3xl">
          ...
          <p className="text-xl md:text-2xl text-green-50 mb-8 font-light">
            Protegemos tu hogar y tu negocio. Especialistas en desinfección y erradicación total.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            {/* Botón Principal Blanco con texto Verde */}
      ...
      <section id="servicios" className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {[
            { title: "Desinsectación", icon: Bug, desc: "Cucarachas, hormigas, arañas y mosquitos. Gel sin olor." },
            { title: "Desratización", icon: Rat, desc: "Control de roedores con estaciones de cebado seguras." },
            { title: "Desinfección", icon: Droplets, desc: "Eliminación de virus y bacterias. Limpieza de tanques." },
          ].map((s, i) => (
      ...
      <section className="bg-green-50 py-16 border-y border-green-100">
        <div className="container mx-auto px-4 text-center">
          ...
          <p className="text-lg text-gray-700 max-w-2xl mx-auto">
            En <strong>{COMPANY_INFO.name}</strong>, todos nuestros trabajos cuentan con garantía escrita y certificados aptos para presentar ante autoridades municipales.
          </p>
        </div>
      </section>
    </div>
    ...
    <footer className="bg-green-900 text-green-100 py-12 mt-auto">
      <div className="container mx-auto px-4 grid md:grid-cols-3 gap-8">
        <div>
          ...
          <p className="text-green-200">Expertos en control de plagas y desinfección integral. Cuidamos lo que más te importa.</p>
        </div>
        <div>
          <h3 className="text-xl font-bold text-white mb-4">Contacto</h3>
          <p className="mb-2">{COMPANY_INFO.address}</p>
          <a href={`tel:${COMPANY_INFO.phone}`} className="block mb-2 hover:text-white transition">{COMPANY_INFO.phone}</a>
...
def update_files():
    print("🎨 Actualizando diseño a SGA (Verde/Blanco)...")
